        ...
        if ("document".equals(pages[currentPageIndex]) && null == documentId) {
            if (JOptionPane.YES_OPTION == Confirm.confirm(InvoiceResources.getString("save_document_question"))) {
                ...
                try {
                    ...
                    /*                    if (0 < mediator.execute("insert into documents(number,type_id,issuer_id,recipient_id,state_id,date_issued,date_due) values(?,?,?,?,?,?,?",
                    row.getValue(1), row.getValue(2), row.getValue(3), row.getValue(4), row.getValue(5), row.getValue(6), row.getValue(7))) {
                    }*/
                    mediator.commit();
