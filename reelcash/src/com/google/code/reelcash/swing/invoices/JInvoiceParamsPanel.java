...
public class JInvoiceParamsPanel extends javax.swing.JPanel {
    ...
    private void computeExchangeRate(Integer bankId) {
        ...
        final String exchangeRate = "select exchange_rates.id, currencies.code, exchange_rates.value from exchange_rates inner join currencies on currencies.id = exchange_rates.currency_id where (currency_id=?) and (bank_id=?) and ((date('now') between start_date and end_date) or permanent=1)";
        try {
            DataRow[] rows = mediator.fetch(exchangeRate, currencyId, bankId);
            if (1 > rows.length) {
