...
public class JDocumentPanel extends javax.swing.JPanel {
    ...
    private boolean validateValues() {
        if (0 > issuerCombo.getSelectedIndex())
            throw new ReelcashException(InvoiceResources.getString("document_issuer_missing"));
        if (0 > recipientCombo.getSelectedIndex())
            throw new ReelcashException(InvoiceResources.getString("document_recipient_missing"));
        if (0 > typeCombo.getSelectedIndex())
            throw new ReelcashException(InvoiceResources.getString("document_type_missing"));
        if (1 > numberField.getText().length())
            throw new ReelcashException(InvoiceResources.getString("document_number_missing"));
        if (null == dateIssuedField.getValue())
            throw new ReelcashException(InvoiceResources.getString("document_issue_date_missing"));
        if (null == dateDueField.getValue())
            throw new ReelcashException(InvoiceResources.getString("document_due_date_missing"));
        return true;
    }
  ...
  private void initComponents() {
    ...
    issuerCombo.addItemListener(new java.awt.event.ItemListener() {
      public void itemStateChanged(java.awt.event.ItemEvent evt) {
        personComboItemStateChanged(evt);
      }
    });
    issuerCombo.addKeyListener(new java.awt.event.KeyAdapter() {
      public void keyReleased(java.awt.event.KeyEvent evt) {
        personComboKeyReleased(evt);
      }
    });
    gridBagConstraints = new java.awt.GridBagConstraints();
    gridBagConstraints.gridx = 1;
    ...
    recipientCombo.addItemListener(new java.awt.event.ItemListener() {
      public void itemStateChanged(java.awt.event.ItemEvent evt) {
        personComboItemStateChanged(evt);
      }
    });
    ...
    private void personComboItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_personComboItemStateChanged
        DataRowComboModel model;
        if (issuerCombo == evt.getSource())
            model = getRecipientModel();
        else if (recipientCombo == evt.getSource())
            model = getIssuerModel();
        else
            return;
        ...
        model.sortBy(1);
    }//GEN-LAST:event_personComboItemStateChanged

    private void personComboKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_personComboKeyReleased
        if (java.awt.event.KeyEvent.VK_BACK_SPACE == evt.getKeyCode())
            ((JComboBox) evt.getSource()).setSelectedIndex(-1);
    }//GEN-LAST:event_personComboKeyReleased
