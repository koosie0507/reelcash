...
/**
 * Insert some documentation for the class <b>MultiLineLabelUI</b> (what's its purpose,
...
public class MultiLineLabelUI extends BasicLabelUI {
    ...
    protected void drawString(Graphics g, String s, int accChar, int textX, int textY) {
        if (s.indexOf('\n') == -1)
            ...
            String[] localStrings = splitStringByLines(s);
            int height = g.getFontMetrics().getHeight();
            // Only the first line can have the accel char
            BasicGraphicsUtils.drawString(g, localStrings[0], accChar, textX, textY);
            for (int i = 1; i < localStrings.length; i++)
                g.drawString(localStrings[i], textX, textY + (height * i));
        }
    }
