...
import com.google.code.reelcash.data.FilterMode;
import com.google.code.reelcash.data.InvoiceDetailsAdapter;
...
public class InvoiceJDialog extends javax.swing.JDialog {
	...
	private PrimaryKeyRow invoiceId;

	/** Creates new form InvoiceJDialog
	 * @param parent
	 * @param currentInvoiceId
	 */
	public InvoiceJDialog(MainFrame parent, PrimaryKeyRow currentInvoiceId) {
		super(parent, true);
		adapter = new InvoicesAdapter();
		initComponents();
		this.invoiceId = currentInvoiceId;
		editMode = (null != invoiceId) && (((Integer) invoiceId.get("invoiceid")) > 0);
	}
  ...
  private void initComponents() {
    ...
    detailsTable.setModel(new javax.swing.table.DefaultTableModel(
      new Object [][] {
        {null, new Integer(1), null, "buc", new Integer(1), null}
      },
      new String [] {
        "invoicedetailid", "Nr. crt", "Denumire", "U.M.", "Cantitate", "Pret Unitar"
      }
    ) {
	...
	private void createInvoice() {
		invoiceId = adapter.create((Integer) invoiceNoSpinner.getValue(),
		...
		for (int row = 0; row < rowCount; row++) {
			adapter.getDetailsAdapter().create(
	...
	private void updateInvoice() {
		adapter.update(invoiceId, (Integer) invoiceNoSpinner.getValue(),
				invoiceSeriesField.getText(),
				SysUtils.getSqlDate((Date) dateSpinner.getValue()),
				SysUtils.getSqlDate((Date) dueDateSpinner.getValue()),
				customerNameField.getText(),
		...
		// TODO: metoda curenta de refacere a detaliilor este foarte urata - de refacut
		((InvoiceDetailsAdapter) adapter.getDetailsAdapter()).deleteAll(invoiceId);

		DefaultTableModel model = (DefaultTableModel) detailsTable.getModel();
		int rowCount = model.getRowCount();
		for (int row = 0; row < rowCount; row++) {
			Integer invoiceid = (Integer) invoiceId.get("invoiceid");
			Integer pos = ((Integer) model.getValueAt(row, 1));
			String name = (String) model.getValueAt(row, 2);
			String um = (String) model.getValueAt(row, 3);
			Integer qty = ((Integer) model.getValueAt(row, 4));
			Double price = ((Double) model.getValueAt(row, 5));
			adapter.getDetailsAdapter().create(invoiceid, pos, name, um, qty, price);
		}
	...
	private void loadDbData() {
		...
		Hashtable<String, Object> invoice = adapter.get(invoiceId);

		invoiceNoSpinner.setValue(invoice.get("number"));
		invoiceSeriesField.setText(invoice.get("series").toString());
		dateSpinner.setValue(invoice.get("invoicedate"));
		dueDateSpinner.setValue(invoice.get("duedate"));
		customerNameField.setText((String) invoice.get("customer"));
		customerAddressField.setText((String) invoice.get("billingaddress"));
		customerRegionField.setText((String) invoice.get("region"));
		customerCountryField.setText((String) invoice.get("country"));
		customerIdCodeField.setText((String) invoice.get("siccode"));
		customerRegNoField.setText((String) invoice.get("commerceregno"));
		customerBankAccountField.setText((String) invoice.get("iban"));
		customerBankField.setText((String) invoice.get("bank"));

		adapter.getDetailsAdapter().populateTableModel((DefaultTableModel) detailsTable.getModel(),
				FilterMode.ALL, new String[]{"invoiceid=?"}, invoiceId.get("invoiceid"));
	}
    ...
    private void onAddInvoiceDetailRequested(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_onAddInvoiceDetailRequested
		DefaultTableModel model = ((DefaultTableModel) detailsTable.getModel());
		model.addRow(new Object[]{-1, model.getRowCount() + 1, "", "buc", 1, null});
	...
	private void onDialogOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_onDialogOpened
		loadDbData();
	}//GEN-LAST:event_onDialogOpened
